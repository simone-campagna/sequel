...
class SearchCatalog(SearchAlgorithm):
    ...
    def _impl_call(self, catalog, items, info, options):
        sub_info = info.sub(rank=1)
        for sequence in catalog.iter_matching_sequences(items):
...
class SearchGeometric(SearchAlgorithm):
    ...
    def _impl_call(self, catalog, items, info, options):
        base = get_base(items)
        if base is not None:
            yield Geometric(base=base), info.sub(rank=2)
...
class SearchAffineGeometric(SearchAlgorithm):
    ...
    def _impl_call(self, catalog, items, info, options):
        ...
        if sequence_matches(sequence, items):
            yield sequence, info.sub(rank=2)
...
class SearchPower(SearchAlgorithm):
    ...
    def _impl_call(self, catalog, items, info, options):
        pwr = get_power(items)
        if pwr is not None:
            yield Power(power=pwr), info.sub(rank=2)
...
class SearchFibonacci(SearchAlgorithm):
    ...
    def _impl_call(self, catalog, items, info, options):
        if items.derivative[1:] == items[:-2]:
            ...
            if fs_gcd != 1:
                fib = fs_gcd * fib
            yield fib, info.sub(rank=1)


class SearchPolynomial(SearchAlgorithm):
    """Search for polynomials"""
    __serialization_keys__ = SearchAlgorithm.__serialization_keys__ + ['max_degree']
    __min_items__ = 3

    def __init__(self, min_degree=3, max_degree=5, name=None):
        super().__init__(name=name)
            ...
            try:
                ...
                if np.all(np.rint(poly) == poly):
                    ...
                    if sequence is not None:
                        ...
                        if sequence_matches(sequence, items):
                            yield sequence, info.sub(rank=degree)
...
class SearchLinearCombination(SearchAlgorithm):
    ...
    __serialization_keys__ = SearchAlgorithm.__serialization_keys__ + [
        'max_items', 'min_components', 'max_components', 'rationals',
        'max_elapsed', 'search_method', 'weight_threshold', 'weight_value',
        'sequences']

    def __init__(self, max_items=19, min_components=2, max_components=4, rationals=True,
                 max_elapsed=3.0, search_method='weighted',
                 weight_threshold=4, weight_value=0.9, sequences=None, name=None):
        super().__init__(name=name)
        if sequences:
        ...
        for indices in iter_combinations(all_indices, num_items, data):
            ...
            for denom, coeffs in solutions:
                ...
                if max_components is None or num_components <= max_components:
                    sub_info = info.sub(rank=num_components)
    ...
    def to_dict(self, serialization):
        ...
        dct["sequences"] = [str(sequence) for sequence in dct["sequences"]]
        return dct
